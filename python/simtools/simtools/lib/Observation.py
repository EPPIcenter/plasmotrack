...
class SimpleGeneticsObservationProcess1(GeneticsObservationProcess):
    ...
    def apply(self, infection: Infection) -> Genotype:
        ...
        for label, locus in flat_genotype.items():
            total_alleles = len(locus)
            obs_locus = []
            for allele in locus:
                if allele == 1:
                    obs_locus.append(self.rng.binomial(1, 1 - (self.fnr / total_alleles)))
                else:
                    obs_locus.append(self.rng.binomial(1, self.fpr / total_alleles))
            out[label] = obs_locus
        return out


class SimpleGeneticsObservationProcess2(GeneticsObservationProcess):
    def __init__(self, fpr: float, fnr: float, seed: int = 0) -> None:
        self.seed = seed
        self.rng = np.random.default_rng(seed=seed)
        self.fpr = fpr
    ...
    def apply(self, infection: Infection) -> Genotype:
        ...
        for label, locus in flat_genotype.items():
            ...
            negative_indices = []
            obs_locus = []
            for idx, allele in enumerate(locus):
                if allele == 1:
                    ...
                    obs_locus.append(0)
            false_positives = set(self.rng.choice(negative_indices, total_false_positives))
