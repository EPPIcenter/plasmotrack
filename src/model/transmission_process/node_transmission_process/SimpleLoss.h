...
namespace transmission_nets::model::transmission_process {
    ...
    class SimpleLoss : public core::computation::Computation<std::array<long double, MAX_TRANSMISSIONS + 1>>,
                       public core::abstract::Observable<SimpleLoss<MAX_TRANSMISSIONS, MAX_PARENTSET_SIZE, InterTransmissionProbImpl>>,
                       public core::abstract::Cacheable<SimpleLoss<MAX_TRANSMISSIONS, MAX_PARENTSET_SIZE, InterTransmissionProbImpl>>,
                       public core::abstract::Checkpointable<SimpleLoss<MAX_TRANSMISSIONS, MAX_PARENTSET_SIZE, InterTransmissionProbImpl>, std::array<long double, MAX_TRANSMISSIONS + 1>> {
...
//        template<typename GeneticsImpl>
//        Likelihood calculateLikelihood(std::shared_ptr<core::containers::Infection<GeneticsImpl>> infection,
//                                          const core::containers::ParentSet<core::containers::Infection<GeneticsImpl>>& parentSet) {
//                return std::exp(calculateLogLikelihood(infection, parentSet));
//        };


        template<typename GeneticsImpl>
        Likelihood peekCalculateLogLikelihood(std::shared_ptr<core::containers::Infection<GeneticsImpl>> infection,
                                          const core::containers::ParentSet<core::containers::Infection<GeneticsImpl>>& parentSet);
...
//                                          const core::containers::ParentSet<core::containers::Infection<GeneticsImpl>>& parentSet);
        ...
        template<typename GeneticsImpl>
        Likelihood peekCalculateLikelihood(std::shared_ptr<core::containers::Infection<GeneticsImpl>> infection,
                                          const core::containers::ParentSet<core::containers::Infection<GeneticsImpl>>& parentSet) {
                return std::exp(peekCalculateLogLikelihood(infection, parentSet));
        };


    private:
        friend class core::abstract::Checkpointable<SimpleLoss<MAX_TRANSMISSIONS, MAX_PARENTSET_SIZE, InterTransmissionProbImpl>, std::array<long double, MAX_TRANSMISSIONS + 1>>;
        friend class core::abstract::Cacheable<SimpleLoss<MAX_TRANSMISSIONS, MAX_PARENTSET_SIZE, InterTransmissionProbImpl>>;
    ...
    template<unsigned int MAX_TRANSMISSIONS, unsigned int MAX_PARENTSET_SIZE, typename InterTransmissionProbImpl>
    SimpleLoss<MAX_TRANSMISSIONS, MAX_PARENTSET_SIZE, InterTransmissionProbImpl>::SimpleLoss(p_ParameterDouble loss_prob, std::shared_ptr<InterTransmissionProbImpl> interTransmissionProb) : lossProb_(std::move(loss_prob)),
    ...
    template<unsigned int MAX_TRANSMISSIONS, unsigned int MAX_PARENTSET_SIZE, typename InterTransmissionProbImpl>
    std::array<long double, MAX_TRANSMISSIONS + 1> SimpleLoss<MAX_TRANSMISSIONS, MAX_PARENTSET_SIZE, InterTransmissionProbImpl>::value() noexcept {
        /*
    ...
    template<unsigned int MAX_TRANSMISSIONS, unsigned int MAX_PARENTSET_SIZE, typename InterTransmissionProbImpl>
    template<typename GeneticsImpl>
    Likelihood SimpleLoss<MAX_TRANSMISSIONS, MAX_PARENTSET_SIZE, InterTransmissionProbImpl>::calculateLogLikelihood(std::shared_ptr<core::containers::Infection<GeneticsImpl>> infection, const core::containers::ParentSet<core::containers::Infection<GeneticsImpl>>& parentSet) {
        ...
        for (const auto& locus : loci) {
            ...
            for (parentIdx = 0; parentIdx < numParents; ++parentIdx) {
                psIdxGen.reset(numParents, parentIdx + 1);
                while (!psIdxGen.completed) {
                    ...
                    for (const auto& g : parentGenotypes) {
                        base = GeneticsImpl::shared(base, g);
                    }
    ...
    template<unsigned int MAX_TRANSMISSIONS, unsigned int MAX_PARENTSET_SIZE, typename InterTransmissionProbImpl>
    template<typename GeneticsImpl>
    Likelihood SimpleLoss<MAX_TRANSMISSIONS, MAX_PARENTSET_SIZE, InterTransmissionProbImpl>::calculateLogLikelihood(std::shared_ptr<core::containers::Infection<GeneticsImpl>> infection, std::shared_ptr<core::containers::Infection<GeneticsImpl>> latentParent, const core::containers::ParentSet<core::containers::Infection<GeneticsImpl>>& ps) {
        std::vector<std::shared_ptr<core::containers::Infection<GeneticsImpl>>> parentSet{ps};
        parentSet.push_back(latentParent);
        size_t numParents = parentSet.size();
        auto loci         = infection->loci();
    ...
    template<unsigned int MAX_TRANSMISSIONS, unsigned int MAX_PARENTSET_SIZE, typename InterTransmissionProbImpl>
    template<typename GeneticsImpl>
    Likelihood SimpleLoss<MAX_TRANSMISSIONS, MAX_PARENTSET_SIZE, InterTransmissionProbImpl>::peekCalculateLogLikelihood(std::shared_ptr<core::containers::Infection<GeneticsImpl>> infection, const core::containers::ParentSet<core::containers::Infection<GeneticsImpl>>& parentSet) {
        size_t numParents = parentSet.size();
        auto loci         = infection->loci();
        ...
        // events are either the loss or retention of alleles.
        for (const auto& locus : loci) {
            auto childGenotype         = infection->latentGenotype(locus)->peek();
            auto invertedChildGenotype = GeneticsImpl::invert(childGenotype);
            auto base                  = invertedChildGenotype;
            size_t parentIdx           = 0;
            ...
            // alleles that are lost in the transmission process.
            for (const auto& parent : parentSet) {
                parentGenotypes[parentIdx] = parent->latentGenotype(locus)->peek();
                allelesLostCounter_[parentIdx] += GeneticsImpl::falseNegativeCount(parentGenotypes[parentIdx], childGenotype);
                base = GeneticsImpl::shared(base, parentGenotypes[parentIdx]);
                ++parentIdx;
            }
