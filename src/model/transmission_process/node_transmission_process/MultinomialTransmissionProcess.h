...
namespace transmission_nets::model::transmission_process {
    ...
    class MultinomialTransmissionProcess2 : public core::computation::Computation<std::array<double, MAX_STRAINS*(MAX_PARENTSET_SIZE + 1)>>,
                                           ...
                                           public core::abstract::Checkpointable<MultinomialTransmissionProcess2<MAX_PARENTSET_SIZE, MAX_STRAINS, SourceTransmissionProcessImpl>, std::array<double, MAX_STRAINS*(MAX_PARENTSET_SIZE + 1)>> {
    ...
    private:
        friend class core::abstract::Checkpointable<MultinomialTransmissionProcess2, std::array<double, MAX_STRAINS*(MAX_PARENTSET_SIZE + 1)>>;
    ...
    template<unsigned int MAX_PARENTSET_SIZE, unsigned int MAX_STRAINS, typename SourceTransmissionProcessImpl>
    std::array<double, MAX_STRAINS*(MAX_PARENTSET_SIZE + 1)> MultinomialTransmissionProcess2<MAX_PARENTSET_SIZE, MAX_STRAINS, SourceTransmissionProcessImpl>::value() {
        /*
        ...
        for (const auto& locus : loci) {
            ...
            for (const auto& parent : parentSet) {
                ...
                for (size_t j = 0; j < parentGenotype.totalAlleles(); ++j) {
                    ...
                    if (parentGenotype.allele(j)) {
                        parent_pop_freqs[j] += (static_cast<double>(parentGenotype.allele(j)) / totalAllelesPresent / (numParents + 1.0));
                    }
                }
            }
            ...
            for (size_t j = 0; j < latentParentGenotype.totalAlleles(); ++j) {
                parent_pop_freqs[j] += (static_cast<double>(latentParentGenotype.allele(j)) / totalAllelesPresent / (numParents + 1.0));
            }
    ...
    Likelihood MultinomialTransmissionProcess2<MAX_PARENTSET_SIZE, MAX_STRAINS, SourceTransmissionProcessImpl>::calculateLogLikelihood(std::shared_ptr<core::containers::Infection<GeneticsImpl>> infection,
        ...
        for (const auto& locus : loci) {
            ...
            for (size_t j = 0; j < parentGenotype.totalAlleles(); ++j) {
                if (parentGenotype.allele(j)) {
                    parent_pop_freqs[j] += (static_cast<double>(parentGenotype.allele(j)) / totalAllelesPresent);
                }
            }
