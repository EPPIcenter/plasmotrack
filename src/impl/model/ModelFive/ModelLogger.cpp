...
namespace transmission_nets::impl::ModelFive {
    ModelLogger::ModelLogger(std::shared_ptr<Model> model, fs::path rootPath) : model_(std::move(model)), rootPath_(std::move(rootPath)) {
        ...
        for (const auto &tp : model_->transmissionProcessList) {
            loggers_.push_back(new core::io::ParentSetDistLogger(tp, std::make_unique<core::io::FileOutput>(parentSetFolder_ / ( core::io::makePathValid(tp->child_->id() + "_ps.csv")), "parent_set,Llik,iter")));
        }
    }
