...
namespace transmission_nets::impl::ModelFive {
    StateLogger::StateLogger(std::shared_ptr<State> state, fs::path rootPath, bool resetOutput) : state_(std::move(state)), rootPath_(std::move(rootPath)) {
        ...
        for (auto &infection : state_->infections) {
            ...
            loggers_.push_back(new core::io::ValueLogger(state_->observationFalsePositiveRates[i], std::make_unique<core::io::FileOutput>(epsPosFolder / inf_file_name, "eps_pos", resetOutput)));
            loggers_.push_back(new core::io::ValueLogger(state_->observationFalseNegativeRates[i], std::make_unique<core::io::FileOutput>(epsNegFolder / inf_file_name, "eps_neg", resetOutput)));
