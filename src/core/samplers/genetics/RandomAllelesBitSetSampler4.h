...
namespace transmission_nets::core::samplers::genetics {
    ...
    private:
        ...
        const double fp_rate = 0.05f;
        const double fn_rate = 0.20f;
        const double tx_from_parent = 0.90f;
        const double tx_from_other = 0.20f;
    };
    ...
    void RandomAllelesBitSetSampler4<T, Engine, InfectionEventImpl, AlleleImpl, LocusImpl>::update() noexcept {
        ...
        double curr_state_log_prob = 0.0f;
        double proposed_state_log_prob = 0.0f;
        ...
        while (distance > 0) {
            ...
            const double obs_data_component = has_data ? (in_curr ? (1 - fp_rate) : (fn_rate)) : 1.0f;
