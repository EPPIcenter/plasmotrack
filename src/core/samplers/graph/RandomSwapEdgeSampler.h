...
public:
    RandomSwapEdgeSampler(TransmissionNetwork<NodeValueImpl> &network, T &target, Engine *rng);
...
RandomSwapEdgeSampler<T, Engine, NodeImpl>::RandomSwapEdgeSampler(TransmissionNetwork<NodeImpl> &network, T &target, Engine *rng) :
...
void RandomSwapEdgeSampler<T, Engine, NodeValueImpl>::update() noexcept {
    ...
    if (nodeA != nodeB) {
        Parameter<ParentSet<NodeValueImpl>>* nodeAParentSetParam = network_.parentSet(nodeA);
        Parameter<ParentSet<NodeValueImpl>>* nodeBParentSetParam = network_.parentSet(nodeB);
