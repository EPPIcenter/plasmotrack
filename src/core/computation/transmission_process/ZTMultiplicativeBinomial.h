...
class ZTMultiplicativeBinomial : public Computation<ProbabilityMatrix <MAX_COUNT>>,
                                 public Observable<ZTMultiplicativeBinomial<MAX_COUNT>>,
                                 public Cacheable<ZTMultiplicativeBinomial<MAX_COUNT>>,
                                 public Checkpointable<ZTMultiplicativeBinomial<MAX_COUNT>, ProbabilityMatrix <MAX_COUNT>> {
public:
    ZTMultiplicativeBinomial(Parameter<double> &prob,
                             Parameter<double> &assoc);

    ProbabilityMatrix <MAX_COUNT> value() noexcept override {
        if (this->isDirty()) {
...
private:
    friend class Checkpointable<ZTMultiplicativeBinomial<MAX_COUNT>, ProbabilityMatrix <MAX_COUNT>>;
    friend class Cacheable<ZTMultiplicativeBinomial<MAX_COUNT>>;

    static const SquareMatrix<double, MAX_COUNT> mat1;
    static const SquareMatrix<double, MAX_COUNT> mat2;
    static const SquareMatrix<double, MAX_COUNT> mat3;
    static const SquareMatrix<double, MAX_COUNT> combo_matrix;
...
template<int MAX_COUNT>
SquareMatrix<double, MAX_COUNT> initMat1() {
    SquareMatrix<double, MAX_COUNT> a;
...
template<int MAX_COUNT>
SquareMatrix<double, MAX_COUNT> initMat2() {
    SquareMatrix<double, MAX_COUNT> a;
    for (int j = 0; j < MAX_COUNT; ++j) {
        for (int k = 0; k <= j; ++k) {
            a(j, k) = j - k;
...
template<int MAX_COUNT>
SquareMatrix<double, MAX_COUNT> initMat3() {
    SquareMatrix<double, MAX_COUNT> a;
    for (int j = 0; j < MAX_COUNT; ++j) {
        for (int k = 0; k <= j; ++k) {
            a(j, k) = (k + 1) * (j - k);
...
template<int MAX_COUNT>
SquareMatrix<double, MAX_COUNT> initCombosMat() {
    SquareMatrix<double, MAX_COUNT> a;
    for (int j = 0; j < MAX_COUNT; ++j) {
        for (int k = 0; k <= j; ++k) {
            a(j, k) = boost::math::binomial_coefficient<double>(j + 1, k + 1);
        }
    }
    return a;
}
...
template<int MAX_COUNT>
const SquareMatrix<double, MAX_COUNT> ZTMultiplicativeBinomial<MAX_COUNT>::mat1 = initMat1<MAX_COUNT>();

template<int MAX_COUNT>
const SquareMatrix<double, MAX_COUNT> ZTMultiplicativeBinomial<MAX_COUNT>::mat2 = initMat2<MAX_COUNT>();

template<int MAX_COUNT>
const SquareMatrix<double, MAX_COUNT> ZTMultiplicativeBinomial<MAX_COUNT>::mat3 = initMat3<MAX_COUNT>();
