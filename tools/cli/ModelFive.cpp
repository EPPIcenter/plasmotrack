...
std::unique_ptr<ReplicaExchange<ModelFive::State, ModelFive::Model, ModelFive::SampleScheduler, ModelFive::ModelLogger, ModelFive::StateLogger>> repex;
...
int main(int argc, char** argv)
{
    ...
    try
    {
        ...
        desc.add_options()
        ("help", "Runs the model five implementation")
        ("burnin,b", po::value<int>(&burnin)->default_value(5000), "Number of steps to be used for burnin")
        ("sample,s", po::value<int>(&sample)->default_value(10000), "Total number of steps to be used for sampling")
        ("thin,t", po::value<int>(&thin)->default_value(1000), "Number of steps to be thinned")
        ("numchains,n", po::value<int>(&numChains)->default_value(1), "Number of chains to run in replica exchange algorithm. Do not exceed the number of threads available.")
        ("gradient,g", po::value<double>(&gradient)->default_value(1), "Temperature gradient to use in replica exchange algorithm")
        ("seed", po::value<int>(&seed)->default_value(-1), "Seed used in random number generator. Note that if numchains > 1 then there is no guarantee of reproducibility. A value of -1 indicates generate a random seed.")
        ("input,i", po::value<std::string>(&input)->required(), "Input file")
        ("hotload,h", "Hotload parameters from the output directory")
        ...
        try
        {
            po::store(po::command_line_parser(argc, argv).options(desc)
                      ...
                      if ( vm.count("help")  )
                      {
                          std::cout << "Model Five Implementation" << std::endl
                          << desc << std::endl;
                          return SUCCESS;
                      }
