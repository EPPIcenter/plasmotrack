...
class spline
{
   ...
   public:
    ...
    void set_boundary(bd_type left, float left_value, bd_type right,
                      float right_value);
    ...
    float solve_one(float y, bool ignore_extrapolation = true) const;
...
std::vector<float> solve_cubic(float a, float b, float c, float d,
                                int newton_iter = 0);
...
void spline::set_points(const std::vector<float> &x,
                        const std::vector<float> &y, spline_type type)
{
    ...
    else if (type == cspline_hermite)
    {
        ...
        else if (m_left == second_deriv)
        {
            ...
            m_b[0] = 0.5 * (-m_b[1] - 0.5 * m_left_value * h +
                            3.0 * (m_y[1] - m_y[0]) / h);
        }
        else if (m_left == not_a_knot)
        {
            // f''' continuous at x[1]
            const float h0 = m_x[1] - m_x[0];
            const float h1 = m_x[2] - m_x[1];
        ...
        else if (m_right == not_a_knot)
        {
            ...
            const float h0 = m_x[n - 2] - m_x[n - 3];
            const float h1 = m_x[n - 1] - m_x[n - 2];
            m_b[n - 1] = -m_b[n - 2] + 2.0 * (m_y[n - 1] - m_y[n - 2]) / h1 +
                         h1 * h1 / (h0 * h0) *
                             (m_b[n - 3] + m_b[n - 2] -
                              2.0 * (m_y[n - 2] - m_y[n - 3]) / h0);
...
bool spline::make_monotonic()
{
    ...
    for (int i = 0; i < n - 1; i++)
    {
                 ...
                 (m_b[i] <= 0.0 && m_b[i + 1] <= 0.0 && avg < 0.0))
        {
            ...
            float r = sqrt(m_b[i] * m_b[i] + m_b[i + 1] * m_b[i + 1]) /
                       std::fabs(avg);
            if (r > 3.0)
            {
...
std::vector<float> spline::solve(float y, bool ignore_extrapolation) const
{
    std::vector<float> x;     // roots for the entire spline
    std::vector<float> root;  // roots for each piecewise cubic
    ...
    for (size_t i = 0; i < n - 1; i++)
    {
        ...
        for (size_t j = 0; j < root.size(); j++)
        {
            float h = (i > 0) ? (m_x[i] - m_x[i - 1]) : 0.0f;
            float eps = internal::get_eps() * 512.0 * std::min(h, 1.0f);
            if ((-eps <= root[j]) && (root[j] < m_x[i + 1] - m_x[i]))
            {
                float new_root = m_x[i] + root[j];
...
float spline::solve_one(float y, bool ignore_extrapolation) const
{
    std::vector<float> root;  // roots for each piecewise cubic
    const size_t n = m_x.size();
...
// machine precision of a float, i.e. the successor of 1 is 1+eps
float get_eps()
{
    // return std::numeric_limits<float>::epsilon();    // __DBL_EPSILON__
    return 2.2204460492503131e-16;  // 2^-52
}

// solutions for a + b*x = 0
std::vector<float> solve_linear(float a, float b)
{
    std::vector<float> x;  // roots
    if (b == 0.0)
    {
        if (a == 0.0)
        {
                                    ...
                                    int newton_iter = 0)
{
    ...
    float q = a / c;
    float discr = p * p - q;
    const float eps = 0.5 * internal::get_eps();
    float discr_err = (6.0 * (p * p) + 3.0 * fabs(q) + fabs(discr)) * eps;
    ...
    for (size_t i = 0; i < x.size(); i++)
    {
        ...
        {
            float f = (c * x[i] + b) * x[i] + a;
            float f1 = 2.0 * c * x[i] + b;
                                ...
                                int newton_iter)
{
    ...
    std::vector<float> z;  // roots of the depressed cubic
    float p = -(1.0 / 3.0) * b + (1.0 / 9.0) * (c * c);
    float r = 2.0 * (c * c) - 9.0 * b;
    float q = -0.5 * a - (1.0 / 54.0) * (c * r);
    float discr = p * p * p - q * q;  // discriminant
    ...
    // would be more elegant to use boost::numeric::interval<float>
    const float eps = internal::get_eps();
    float p_err =
        eps * ((3.0 / 3.0) * fabs(b) + (4.0 / 9.0) * (c * c) + fabs(p));
    float r_err = eps * (6.0 * (c * c) + 18.0 * fabs(b) + fabs(r));
    float q_err = 0.5 * fabs(a) * eps +
                   (1.0 / 54.0) * fabs(c) * (r_err + fabs(r) * 3.0 * eps) +
                   fabs(q) * eps;
    float discr_err = (p * p) * (3.0 * p_err + fabs(p) * 2.0 * eps) +
    ...
    else if (discr > 0)
    {
        ...
        z.resize(3);
        float ac = (1.0 / 3.0) * acos(q / (p * sqrt(p)));
        float sq = 2.0 * sqrt(p);
        z[0] = sq * cos(ac);
    ...
    else if (discr < 0.0)
    {
        ...
        z.resize(1);
        float sgnq = (q >= 0 ? 1 : -1);
        float basis = fabs(q) + sqrt(-discr);
        float C = sgnq * pow(basis, 1.0 / 3.0);  // c++11 has std::cbrt()
        z[0] = C + p / C;
    }
    for (size_t i = 0; i < z.size(); i++)
    {
        ...
        for (int k = 0; k < newton_iter; k++)
        {
            float f = ((z[i] + c) * z[i] + b) * z[i] + a;
            float f1 = (3.0 * z[i] + 2.0 * c) * z[i] + b;
            // only adjust if slope is large enough
            if (fabs(f1) > 1e-8)
            {
                z[i] -= f / f1;
