...
TEST(ModelFourTest, CoreTest) {
    ...
    //    using InformedGeneticsSampler = genetics::ZanellaAllelesBitSetSampler<ModelFour, boost::random::mt19937, ModelFour::GeneticsImpl>;
    ...
    const fs::path nodesFile{"resources/JSON/nodes5.json"};
    ...
    for (auto &infFNR : model.state.observationFalseNegativeRates) {
        scheduler.registerSampler({.sampler = new ConstrainedContinuousRandomWalk(infFNR, model, 0.0, 0.2, &r, .1),
                                   //                               .adaptationStart = 1000,
                                   //                               .adaptationEnd = 3000,
                                   //                                          .updateStart = 1000,
                                   .weight = 3});
    ...
    for (auto &infFPR : model.state.observationFalsePositiveRates) {
        scheduler.registerSampler({.sampler = new ConstrainedContinuousRandomWalk(infFPR, model, 0.0, 0.1, &r, .1),
                                   //                               .adaptationStart = 1000,
                                   //                               .adaptationEnd = 3000,
                                   //                                          .updateStart = 1000,
                                   .weight = 3});
