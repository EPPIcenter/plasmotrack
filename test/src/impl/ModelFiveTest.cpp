...
TEST(ModelFiveTest, CoreTestFull) {
    ...
    auto infections = parseInfectionsFromJSON<InfectionEvent, Locus>(j, ModelFive::MAX_COI, loci);
    auto allowedParents = parseAllowedParentsFromJSON(j, infections);

    unsigned seed = std::chrono::system_clock::now().time_since_epoch().count();
    auto rng = std::default_random_engine{seed};
    std::shuffle(infections.begin(), infections.end(), rng);

    ModelFive::Model model(loci, infections, allowedParents);
