...
//    loggers.push_back(new ValueLogger(state.observationFalsePositiveRate, std::make_unique<FileOutput>(paramOutput / "fpr.csv")));
//    loggers.push_back(new ValueLogger(state.observationFalseNegativeRate, std::make_unique<FileOutput>(paramOutput / "fnr.csv")));
//    loggers.push_back(new ValueLogger(state.geometricGenerationProb, std::make_unique<FileOutput>(paramOutput / "geo_gen_prob.csv")));
//    loggers.push_back(new ValueLogger(state.lossProb, std::make_unique<FileOutput>(paramOutput / "loss_prob.csv")));
//    loggers.push_back(new ValueLogger(state.mutationProb, std::make_unique<FileOutput>(paramOutput / "mutation_prob.csv")));
//    loggers.push_back(new ValueLogger(state.meanCOI, std::make_unique<FileOutput>(paramOutput / "mean_coi.csv")));
//    loggers.push_back(new ValueLogger(model, std::make_unique<FileOutput>(paramOutput / "likelihood.csv")));
//    loggers.push_back(new LambdaLogger([&](){ return state.transmissionNetwork.serialize(); }, std::make_unique<FileOutput>(paramOutput / "network.csv")));
//    for(const auto& [locus_label, locus] : state.loci) {
//        loggers.push_back(new ValueLogger(state.alleleFrequencies.alleleFrequencies(locus), std::make_unique<FileOutput>(paramOutput / (locus_label + "_frequencies.csv"))));
//    }
//
